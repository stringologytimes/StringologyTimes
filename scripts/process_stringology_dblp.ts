...
function createArxivYearMonthMD(_papers: DBLPElement[], year : number) : string[] {
    const lines: string[] = new Array();
    for(var i = 12; i >= 1 ;i--){
        const list = _papers.filter((v) => v.mdate.getFullYear() == year && (v.mdate.getMonth()+1) == i);
        if(list.length > 0){
            lines.push(`### ${year}/${i}  `);
            list.forEach((v, x) =>{
...
function createArxivMD(_papers: DBLPElement[]) : string[] {
    ...
    const yearList : number[] = [...new Set(_papers.map((v) => v.mdate.getFullYear()))];
    yearList.sort((a, b) =>{
        return a > b ? -1 : 1;
    })
    yearList.forEach((year) =>{
        lines.push(`## ${year}  `)
        const spapers = _papers.filter((v) => v.mdate.getFullYear() == year);
